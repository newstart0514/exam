    ...
    <div class="card">
      <a-row :gutter="16">
        <a-col :span="24">
          <div class="item" style="margin-top: 1.5rem; margin-bottom: 1.5rem">
            ...
            <div class="func">
              <a-button status="danger" style="margin-right: 2rem"
                >我要交卷</a-button
              >
            </div>
          </div>
          ...
          >
            <template #columns>
              ...
              ><a-table-column title="试题数量" align="center">
                <template #cell="{ record }"> 共{{ record.sum }}分 </template>
              </a-table-column>
      ...
      <a-row :gutter="16">
        <a-col :span="24">
          ...
          <div class="content">
            1.
            保持诚信：在考试中，要保持诚信，不要试图通过作弊手段来提高分数，否则可能会受到考试主办方的拉黑以及成绩作废处理。
          </div>
          <div class="content">
            2.
            保护个人信息：在网络考试过程中，要注意保护自己的个人信息，如手机号码、邮箱等，避免泄露给他人。
          </div>
          <div class="content">
            3.
            如果遭遇了断网、死机等问题，您可以修复后继续回到页面答题，或者联系考试主办方进行协调。
          </div>
          <div class="content">
            4. 考试禁止跳出页面：考试跳出页面三次将自动交卷。
          </div>
          <div class="content">5. 考试期间如果您需要离开，请向监考官示意。</div>
          <div class="content">6. 如遇到其他问题，请向考试主办方报告。</div>
        </a-col>
      </a-row>
    </div>
  </div>
...
const getQuestion = async () => {
  ...
  const { ok, results, sums, ids } = await httpGet(
    `/api/exam/examInfo?id=${query.id}`,
  );
  if (ok) {
