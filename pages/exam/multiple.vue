    ...
    <div class="right">
      <div class="contentCard">
        ...
        <div style="max-height: 40%;margin-top: 1.5rem;max-width: 100%;">
          <div v-for="item in qaInfo.qaContent" :key="item.text">
            <mul-btn 
              :title="item.title" 
              :content="item.text" 
              :active="choice.indexOf(item.title) !== -1"
              @click="choose(item)"
        ...
        <div style="max-height: 10%;" class="footer">
          <a-button @click="prev"><icon-left />&nbsp;{{index === 0 ? '返回' : '上一题'}}</a-button>
          <a-button @click="next">{{index === count - 1 ? '提交该题型' : '下一题'}}&nbsp;<icon-right /></a-button>
        </div>
      </div>
    </div>
  </div>
...
const tabChoose = (num) => {
  navigateTo(`/exam/multiple?index=${num - 1}`)
  index.value = num - 1
  getQuestion()
}
const saveAnswer = async () => {
  if(preChoice !== choice.value) {
    const id = localStorage.getItem('multiple')?.split(',')[index.value]
    const answer = {
      qaId: parseInt(id),
      answer: choice.value
    }
...
const next = () => {
  ...
  if(index.value !== count.value - 1) {
    index.value++
    navigateTo(`/exam/multiple?index=${index.value}`)
    getQuestion()
  } else {
...
const getCount = () => {
  const Arr = localStorage.getItem('multiple')?.split(',')
  count.value = Arr?.length
}
const choose = (item) => {
  const choiceArr = choice.value.split('')
  const index = choiceArr.indexOf(item.title);
  if(index === -1) {
    choiceArr.push(item.title)
  } else {
    choiceArr.splice(index, 1);
  }
  choice.value = choiceArr.sort().join('')
}
