<template>
  <div style="margin: 1rem 2rem;">
    ...
    >
      <template #extra>
        <a-button type="text" status="success" @click="navigateTo('/adminer/questionDetail')">
          <template #icon>
            <icon-plus />
          </template>
          添加题目
      ...
      >
        <template #columns>
          ...
          <a-table-column title="题目" data-index="qaTitle" align="center" :width="600" ellipsis tooltip></a-table-column>
...
const getList = async () => {
  const {result, sums} = await httpGet(`/api/questionList?page=${pagination.value.current}&size=${pagination.value.pageSize}`)
  data.value = result
