<template>
  <div style="margin: 1rem 2rem">
    <a-card class="card" title="考试列表" :bordered="false">
      <template #extra>
        <a-button
          type="text"
          status="success"
          @click="navigateTo('/adminer/examDetail')"
        >
          <template #icon>
            <icon-plus />
          </template>
...
const getList = async () => {
  const { result, sums } = await httpGet(
    `/api/examList?page=${pagination.value.current}&size=${pagination.value.pageSize}`,
  );
  data.value = result;
