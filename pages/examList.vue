<template>
    <div class="mt ml mr">
      <div>
        ...
        <span style="float: right;margin-right: 0.5rem;"><time-cur></time-cur></span>
      </div>
      <a-divider></a-divider>
      ...
      <a-result status="404" subtitle="没有找到考试，请您和管理员确认考试是否已经被成功添加" style="height: calc(60vh + 7.5rem);" v-else>
        <template #extra>
          <!-- <a-space>
...
const getList = async () => {
  const {result, sums} = await httpGet(`/api/exams?page=${page.value}&size=3`)
  data.value = result
  count.value = sums
